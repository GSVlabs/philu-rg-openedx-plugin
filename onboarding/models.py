...
from custom_fields.multiselect_with_other.db.fields import MultiSelectWithOtherField
...
class OrganizationPartner(models.Model):
    ...
    """
    organization = models.ForeignKey(Organization, related_name='organization_partners', on_delete=models.DO_NOTHING)
...
class GranteeOptIn(models.Model):
    ...
    organization_partner = models.ForeignKey(OrganizationPartner, related_name='grantee_opt_in', on_delete=models.DO_NOTHING)
    user = models.ForeignKey(User, related_name='grantee_opt_in', on_delete=models.DO_NOTHING)
...
class OrganizationAdminHashKeys(TimeStampedModel):
    ...
    """
    organization = models.ForeignKey(Organization, related_name='suggested_admins', on_delete=models.DO_NOTHING)
    suggested_by = models.ForeignKey(User, on_delete=models.DO_NOTHING)
...
class UserExtendedProfile(TimeStampedModel):
    ...
    user = models.OneToOneField(User, unique=True, db_index=True, related_name='extended_profile', on_delete=models.CASCADE)
    ...
    def get_user_selected_functions(self, _type='labels'):
        ...
        """
        if _type == 'labels':
            return map(FUNCTIONS_DICT.get, self.function_areas)
    ...
    def get_user_selected_interests(self, _type='labels'):
        ...
        """
        if _type == 'labels':
            return map(INTERESTS_DICT.get, self.interests)
...
class EmailPreference(TimeStampedModel):
    user = models.OneToOneField(User, related_name="email_preferences", on_delete=models.CASCADE)
...
class OrganizationMetric(TimeStampedModel):
    ...
    """
    ACTUAL_DATA_CHOICES = (
        (False, "Estimated - My answers are my best guesses based on my knowledge of the organization"),
        (True, "Actual - My answers come directly from my organization's official documentation")
    )

    org = models.ForeignKey(Organization, related_name="organization_metrics", on_delete=models.DO_NOTHING)
    user = models.ForeignKey(User, related_name="organization_metrics", on_delete=models.DO_NOTHING)
    submission_date = models.DateTimeField(auto_now_add=True)
    actual_data = models.BooleanField(choices=ACTUAL_DATA_CHOICES, blank=True, null=True)
...
class OrganizationMetricUpdatePrompt(models.Model):
    org = models.ForeignKey(Organization, related_name="organization_metrics_update_prompts", on_delete=models.DO_NOTHING)
    responsible_user = models.ForeignKey(User, related_name="organization_metrics_update_prompts", on_delete=models.DO_NOTHING)
...
class MetricUpdatePromptRecord(TimeStampedModel):
    ...
    CLICK_CHOICES = (
        (REMIND_ME_LATER_KEY, gettext_noop(REMIND_ME_LATER_VAL)),
        (TAKE_ME_THERE_KEY, gettext_noop(TAKE_ME_THERE_VAL)),
        (NOT_INTERESTED_KEY, gettext_noop(NOT_INTERESTED_VAL))
    )
