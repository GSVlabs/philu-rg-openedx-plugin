...
from custom_fields.multiselect_with_other.forms.fields import MultiSelectWithOtherFormField
from custom_fields.multiselect_with_other.helpers import add_other_field_in_choices
...
class OtherMultiSelectFieldList(MSFList):
    def __str__(self):
        ...
        return ', '.join([str(s) for s in selected_choice_list])
...
class MultiSelectWithOtherField(MultiSelectField):
    ...
    def _check_other_max_length_attribute(self):
        ...
        elif not isinstance(self.other_max_length, int) or self.other_max_length <= 0:
            return [
                checks.Error(
